...
Press **Win + `** (backtick) and whatever is on your clipboard gets replaced with its plain-text version. A brief overlay confirms the action, then fades out. Like pasting through Notepad, without the Notepad.

## Features

- **Global hotkey** (Win + `) strips rich formatting from the clipboard
- **Visual confirmation**: centered overlay with your Windows accent color, checkmark, and "Clipboard cleaned" text (400ms fade)
...
- GDI rendering for the overlay, Shell_NotifyIcon for the tray
- DPI-aware via embedded PerMonitorV2 manifest
- Accent color read from DwmGetColorizationColor
